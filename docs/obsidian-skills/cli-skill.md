...
The CLI skill is implemented via the `VaultOps` port (`src/ports/vault-ops.ts`) and its production adapter `ObsidianCliAdapter` (`src/adapters/obsidian-cli.ts`). Agent and command code never calls `obEval` directly — all vault I/O goes through the `VaultOps` interface, which decouples commands from the Obsidian CLI binary. See `docs/adapter-pattern.md` for the full contributor guide.
