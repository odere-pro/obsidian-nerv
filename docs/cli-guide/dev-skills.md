...
All vault I/O operations in commands go through the `VaultOps` interface (`src/ports/vault-ops.ts`), not through `obEval` directly. When extending the skill surface with a new vault operation, follow these steps:

1. Add the method signature to `VaultOps` in `src/ports/vault-ops.ts`.
2. Implement it in `ObsidianCliAdapter` in `src/adapters/obsidian-cli.ts` (use `encodeForJs` for every string argument in `obEval` expressions).
...
See `docs/adapter-pattern.md` for the full guide including architecture diagram, method reference tables, and testing patterns.
