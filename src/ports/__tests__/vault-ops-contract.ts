...
const VAULT = 'contract-test-vault';

export function runVaultOpsContractTests(
  label: string,
  factory: () => VaultOps | Promise<VaultOps>,
  options?: { seedFile?: (ops: VaultOps, vault: string, path: string, content: string, fm?: Record<string, unknown>) => Promise<void> }
): void {
  const seed = options?.seedFile ?? (async (ops, vault, path, content, fm) => {
