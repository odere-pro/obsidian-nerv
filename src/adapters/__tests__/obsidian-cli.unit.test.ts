...
const mockObEval = mock<(vault: string, expr: string) => Promise<string>>();
const mockDailyAppend = mock<(vault: string, content: string) => Promise<void>>();
...
describe('ObsidianCliAdapter', () => {
  ...
  test('openDaily calls spawnCapture with obsidian daily', async () => {
    ...
    expect(mockSpawnCapture.mock.calls[0][0]).toEqual(['obsidian', 'daily', 'vault=test-vault']);
  });
