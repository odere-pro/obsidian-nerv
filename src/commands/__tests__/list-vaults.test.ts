...
let mockVaults: Array<{ name: string; path: string; isDefault?: boolean }> = [];

mock.module('../../lib/vault-registry', () => ({
  readRegistry: async (): Promise<{ vaults: typeof mockVaults }> => ({ vaults: mockVaults }),
}));

const { default: command } = await import('../list-vaults');
...
describe('list-vaults', () => {
  ...
  test('prints aligned table with default marker for populated registry', async () => {
    mockVaults = [
      { name: 'my-vault', path: '/Users/me/git/project/my-vault', isDefault: true },
      { name: 'work-vault', path: '/Users/me/git/project/work-vault' },
    ];
  ...
  test('--json emits parseable JSON array with both entries', async () => {
    mockVaults = [
      { name: 'my-vault', path: '/abs/path/my-vault', isDefault: true },
      { name: 'other', path: '/abs/path/other' },
    ];
